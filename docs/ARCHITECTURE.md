...
`AppShell` owns the shared data provider, accessible skip link, responsive primary navigation, first-visit synchronization status, footer, and desktop status bar. The status waits for a usable feed, reports partial or retained data honestly, and briefly replaces the redundant header feed/clock instruments instead of covering active route content or changing the header height. Its compact mobile and full desktop labels expose the same accessible message under reduced motion, and dismissal returns keyboard focus to the active route. The footer identifies its age and refresh action as schedule-specific so the independent History archive status and refresh control cannot be mistaken for the shared upcoming feed. The desktop ticker keeps its concise visual `T−` notation while exposing the complete, correctly pluralized duration to assistive technology. The shared UTC and countdown clock pauses its one-second cadence while the document is hidden, then resynchronizes immediately when the tab becomes visible so foreground telemetry never waits for a throttled background timer. Detail views report whether their resolved mission belongs to the active schedule or completed archive, so direct canonical links keep the correct Home or History parent current while explicit Watch and History return context still takes precedence. Home, Watch, History, and detail views present route-specific controls without duplicating the global shell. External coverage, intelligence, provider, source, and calendar-handoff actions retain concise visual labels while adding a shared screen-reader-only new-tab cue. An increased-contrast preference strengthens secondary telemetry, panel boundaries, and persistent selected surfaces through shared semantic tokens without replacing the green, magenta, cyan, amber, and red signal language. In forced-colors mode, the shell gives the current route and every `aria-pressed` selection an explicit system-color outline, preserving active navigation and mission/map state after authored fills and inset shadows are removed.
...
- **Home** establishes one visual priority: active coverage first, otherwise the next launch. A live provider broadcast keeps the target countdown visible and is labeled `Coverage live`; only an explicit provider in-flight state becomes `In flight`. Its primary mission summary exposes a valid provider launch window beside the target time instead of reducing a range to one instant. UTC remains authoritative while the shared window primitive derives a compact duration and switches in place between distinct viewer and site-local intervals, preserving the schedule's first-viewport priority. When T-0 passes, shared countdowns use that validated window to distinguish a currently open window from an amber wait for provider confirmation; a target alone never invents an open window. Calendar-ready provider windows of no more than 24 hours also produce a nearest-concurrency planning signal when their intervals overlap; later intersections remain available through an in-panel disclosure, the signal derives from the filtered result set, coarse or malformed windows are excluded, and degraded feeds qualify the result. The URL-backed `ready=1` schedule filter uses the same exact-or-minute precision rule as calendar and alert actions, so calendar-ready missions can be isolated independently of provider status and retained across mission-detail return navigation. The URL-backed `horizon=7d` filter uses a rolling seven-day boundary and accepts day-or-better provider precision while retaining an active mission; month and broader placeholder dates never become false near-term commitments. When a provider supplies both a `Vehicle | Mission` designation and an identical structured mission name, the primary heading removes the redundant vehicle prefix while the telemetry retains the vehicle and compact rows retain the complete provider title. Upcoming missions use readable rows and progressive loading. Detail returns restore the batch containing the selected mission and return keyboard focus to its link. The map appears beside the hero on wide screens and behind an explicit disclosure on narrower screens.
...
- **History** provides search, provider/year/outcome filters, a newest/oldest chronology control for the visible feed window, expandable visual summaries, and stable links to details and available replays. Repeated `View mission` actions keep that concise visual label while including the row's mission name in their accessible name, so assistive-technology link lists remain distinguishable. Failed missions expose the bounded provider diagnosis in their expanded record instead of reducing the outcome to a generic badge. A past-window mission whose provider has not published success or failure is labeled `Outcome unconfirmed`, uses the amber caution treatment, and remains isolatable through the URL-backed Unconfirmed outcome filter instead of inheriting a green success signal from a stale provider status. Filter and chronology context is bounded in the URL and survives mission-detail return navigation; the archive also restores the selected mission's result batch and link focus. When a compact archive record does not include coverage, expansion checks that one canonical detail route on demand and reports checking, a visible amber failure with a focus-stable retry, unavailable, or confirmed-replay state without inflating the 100-record archive request. Its controls expose the current feed's oldest and newest records, and identify when the 100-mission response cap is full, so archive searches never imply unbounded retention.

Launch-site atlas responses cross the same browser trust boundary as launch
feeds. The client requires a complete metadata envelope plus unique site IDs,
bounded coordinates and counts, safe HTTPS handoffs, and structurally valid
agency and image facts before facilities can reach map geometry. A malformed
successful response enters the atlas's retryable degraded state while the open
map and current mission coordinates remain available.
- **Detail** resolves current and completed missions with the same layout and actions, keeps valid provider launch windows visible in the primary mission summary, presents one eligible vehicle or mission visual before telemetry and trajectory, then adapts countdown, timeline, video, and return navigation to mission state. Its map bundle waits until a stable, focusable trajectory loading panel approaches the viewport, keeping the section index useful while reducing initial route work. Horizontally scrollable provider timelines expose their currently readable event range and total as the viewport changes.
