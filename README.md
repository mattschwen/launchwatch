...
- **Watch** selects a mission with active coverage when one exists and otherwise presents the next scheduled mission, its provider fallback, licensed imagery when coverage cannot play in-app, countdown, selected-mission trajectory, and truthful coverage signal. A completed mission-detail check distinguishes an unannounced stream from coverage that is still loading or unavailable, so the settled standby console never implies provider work is still in progress. A sticky console index moves keyboard and touch users directly among coverage, the mission queue, selected-mission details, intelligence, and trajectory while preserving the selected mission URL; decorative channel numbers collapse before narrow labels can clip, and enlarged text exposes the visible range plus previous/next controls instead of silently hiding destinations inside the scroll rail. Its bounded mission queue keeps the active item visibly marked as the mission on console, names every provider state alongside its signal color, and hands larger feeds directly to the batched Home schedule. On wide screens, schedule source and status stay in the mission rail beside the queue while intelligence uses the full console width; mobile preserves the same source context between mission detail and intelligence. Settled coverage and visual detail remain in place while the same mission revalidates after a reconnect, avoiding a false fallback or lost keyboard focus. The browser tab leads with confirmed `COVERAGE LIVE` or `IN FLIGHT` state while a stale live signal is explicitly marked `LAST KNOWN`.
- **History** loads completed missions across connected providers through the internal server API and supports search, provider, year, confirmed or unconfirmed outcome, expandable visual summaries, replay links, and mission details. Provider-backed detail routes load only after a mission is activated, avoiding speculative lookups across the dense archive. Replay verification failures stay visibly amber and retryable instead of disappearing behind an unlabeled action; a completed check with no verified replay offers an explicitly labeled mission-specific search rather than a dead end. Past-window records without a terminal provider result stay visibly amber instead of implying success. In-flight records stay on Home and Watch until provider coverage is no longer live.
