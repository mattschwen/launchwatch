...
`AppShell` owns the shared data provider, accessible skip link, responsive primary navigation, first-visit synchronization status, footer, and desktop status bar. The status waits for a usable feed, reports partial or retained data honestly, and briefly replaces the redundant header feed/clock instruments instead of covering active route content or changing the header height. Its compact mobile and full desktop labels expose the same accessible message under reduced motion, and dismissal returns keyboard focus to the active route. Detail views report whether their resolved mission belongs to the active schedule or completed archive, so direct canonical links keep the correct Home or History parent current while explicit Watch and History return context still takes precedence. Home, Watch, History, and detail views present route-specific controls without duplicating the global shell.
